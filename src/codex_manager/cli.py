...
def _ensure_cloud_archive(args: Any) -> None:
    ...
    cp = get_cloud_provider(args)
    if not cp:
        console.print("[bold red]Error:[/] Could not resolve Cloud (B2) credentials.")
        sys.exit(1)
    ...
    if not email and args.command == "use":
        ...
        if not entries:
            console.print("[bold red]Error:[/] No backups found in Cloud.")
            sys.exit(1)
        rec = choose_best_account(evaluate_records(entries))
        console.print(f"Automatically recommended from Cloud: [bold cyan]{rec.selected.email}[/]")
        email = rec.selected.email
        args.email = email
    
    if not email:
        console.print("[bold red]Error:[/] --email or --from-archive is required for cloud restore/use.")
        sys.exit(1)

    entries = list_cloud_backups(cp, email=email, latest_per_email=True)
    if not entries:
        console.print(f"[bold red]Error:[/] No backups found in Cloud for {email}.")
        sys.exit(1)
    ...
    console.print(f"Downloading from Cloud: [cyan]{archive_name}[/] ...")
...
def main() -> None:
    ...
    if args.command == "backup":
        ...
        if getattr(args, "cloud", False) and not args.dry_run:
            cp = get_cloud_provider(args)
            if cp:
                console.print(f"Uploading to Cloud: [cyan]{archive_path.name}[/] ...")
                cp.upload_file(archive_path, archive_path.name)
                cp.upload_file(metadata_path, metadata_path.name)
                console.print("[bold green]Cloud upload complete.[/]")
            else:
                console.print("[bold red]Error:[/] Could not resolve Cloud (B2) credentials for upload.")
    ...
    if args.command == "profile":
        if args.action == "export":
            ...
            console.print(f"Profile exported to [cyan]{args.file}[/]")
        elif args.action == "import":
            import_profile(Path(args.file).expanduser())
            console.print(f"Profile imported from [cyan]{args.file}[/]")
        return
